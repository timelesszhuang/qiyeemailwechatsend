...
class Getsetneteasemail extends Controller
{
    ...
    public function index()
    {
        ...
        // 循环获取所有符合规定的 客户的绑定信息
        $coms = Db::name('corp_bind_api')->where('status', 'neq', 'off')->where('api_status', '10')->field('corpid,corp_id,privatesecret,product,domain,corp_name,status,flag,api_status,addresslist_show')->select();
        // 获取全部数据
        $email_agentid = Config::get('common.EMAILAGENT_ID');
        foreach ($coms as $corp_bind) {
    ...
    private function formatWechatSendeMail($con, $accounts, $wechat_userid, $agent_id, $corpid)
    {
        ...
        foreach ($list as $k => $v) {
            ...
            if ($result == 1) {
                ...
                $this->redisClient->lpush($this->mailQueue, $data);
            }
        }
        return $total;
    }
