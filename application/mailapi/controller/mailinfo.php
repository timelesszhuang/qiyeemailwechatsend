...
class mailinfo
{
     ...
     * @return array
     */
    public static function get_account_info($prikey, $domain, $product, $flag = '10', $account)
    {
        $time = date(time()) . '000';
        try {
            $res = openssl_pkey_get_private($prikey);
            //需要逐条获取部门信息
            //必须使用post方法
            $src = "account_name=" . $account . "&domain=" . $domain . "&product=" . $product . "&time=" . $time;
            if (openssl_sign($src, $out, $res)) {
                $sign = bin2hex($out);
                if ($flag == '10') {
                    //华北
                    ...
                    //华东
                    $url = "https://apihz.qiye.163.com/qiyeservice/api/account/getAccount";
                }
                $response_json = json_decode(common::send_curl_request($url, $src . '&sign=' . $sign), true);
                if ($response_json['suc']) {
                    return [$response_json['con'], true];
                }
                file_put_contents('account.txt', print_r($response_json, true), FILE_APPEND);
            }
        } catch (Exception $ex) {
            file_put_contents('account.txt', '获取邮件信息错误：' . print_r($ex->getMessage(), true), FILE_APPEND);
        }
        return [[], false];
    }
